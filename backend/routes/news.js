...
async function ensureUniqueSlug(baseSlug, excludeId) {
  let cleanBase =
    String(baseSlug || "").replace(/^new-\d{8,}-?/, "") || "news";
...
async function findArticleBySlug(slugParam, isAdmin) {
  ...
  article = await Article.findOne({
    ...filter,
    legacySlugs: slugParam,
  });

  if (article) return article;

  // 3. Remove old "new-123..." prefix
  ...
  if (stripped && stripped !== slugParam) {
    ...
    article = await Article.findOne({
      ...filter,
      legacySlugs: stripped,
    });

    if (article) return article;
  }

  // 4. Decode URL encoded Malayalam / Unicode slug
  try {
...
// ============================================================
// GET NEWS BY SLUG
// Supports English + Malayalam + legacy slugs
// ============================================================
...
router.get("/slug/:slug", async (req, res) => {
  try {
    ...
    // Do NOT reject Malayalam/Unicode slugs here.
    // findArticleBySlug() handles English, Malayalam,
    // legacy and encoded slugs.
...
router.get("/:slug", async (req, res) => {
  try {
    ...
    // Do NOT validate the slug as English only.
    // Old Malayalam URLs must also be allowed.
    const article = await findArticleBySlug(
      req.params.slug,
      isAdmin
    );
...
router.post("/", authMiddleware, async (req, res) => {
  try {
    ...
    // English title -> English slug
    const slugSource = await englishSlugSource(
      title,
      titleEn,
      requestedSlug
    );
    ...
    const sanitizedVideos =
      sanitizeRelatedVideos(relatedVideos);

    const safeVideoUrl = isSafeVideoUrl(videoUrl)
...
router.put("/:id", authMiddleware, async (req, res) => {
  try {
    const filter = req.params.id.match(
      /^[0-9a-fA-F]{24}$/
    )
      ? { _id: req.params.id }
      : { slug: req.params.id };
    ...
    const slugFieldProvided =
      req.body.slug !== undefined ||
      req.body.engSlug !== undefined;
    ...
    if (slugFieldProvided) {
      ...
      if (
        ...
        const titleEnForSlug =
          req.body.titleEn !== undefined
            ? req.body.titleEn
            : existing.titleEn;

        if (effectiveRequested) {
          slugSource = await englishSlugSource(
            titleForSlug,
            titleEnForSlug,
            effectiveRequested
          );
        ...
        if (slugSource.englishTitle) {
          updateData.titleEn =
            slugSource.englishTitle;
        }

        // Save previous slug as legacy slug
    ...
    if (req.body.relatedVideos !== undefined) {
      updateData.relatedVideos =
        sanitizeRelatedVideos(
          req.body.relatedVideos
        );
    }

    if (req.body.videoUrl !== undefined) {
      updateData.videoUrl = isSafeVideoUrl(
        req.body.videoUrl
      )
    ...
    // Prevent client from overwriting legacy slugs
    ...
    if (computedLegacy) {
      updateData.legacySlugs = computedLegacy;
    }
    ...
    try {
      ...
      for (const article of articles) {
        ...
        if (isOldSlug) {
          try {
            slugSource =
              await englishSlugSource(
                article.title,
                article.titleEn,
                "",
                {
                  forceTitleTranslation: true,
                }
              );
