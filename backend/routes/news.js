...
router.post("/", authMiddleware, async (req, res) => {
  try {
    const { title, titleEn, engSlug: manualEngSlug, category, categories, subcategory, content, excerpt, image, tags, featured, breaking, published, author, body, media, videoUrl, relatedVideos, categoryMl, readTime, backgroundColor, likes, views, mainNews, popular } = req.body;
    if (!title || !category || !content) {
      return res.status(400).json({ error: "title, category, and content are required" });
    }

    let engSlug;
    if (manualEngSlug && manualEngSlug.trim()) {
      engSlug = await ensureUniqueSlug(slugify(manualEngSlug));
    } else {
...
router.put("/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (req.body.engSlug && req.body.engSlug.trim()) {
      ...
      updateData.slug = updateData.engSlug;
...
router.post("/migrate-engslug", authMiddleware, async (req, res) => {
  try {
    ...
    for (const article of articles) {
      ...
      await Article.updateOne({ _id: article._id }, { $set: { engSlug, slug: engSlug } });
      updated++;
    }
