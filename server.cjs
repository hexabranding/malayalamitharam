...
const articlePageHandler = async (req, res) => {
  ...
  if (!article.engSlug && article.title && Article.db.readyState === 1) {
    try {
      ...
      const candidate = r.toLowerCase().replace(/-+/g, "-").replace(/^-+|-+$/g, "").slice(0, 120) || ("post-" + Date.now().toString(36));
      let engSlug = candidate;
      let counter = 2;
      while (await Article.findOne({ engSlug, _id: { $ne: article._id } })) {
        engSlug = candidate + "-" + counter;
        counter++;
      }
